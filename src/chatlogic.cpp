            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // always pass unique ptr as reference only -- makes sense !!!!
                        // will have to use std::move to pass owernership of unique pointer
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            /* use raw pointers when there is no underlysing change in ownership of
                             smart pointers but involves modification of only data on heap.
                            */
                            ...
                            (*parentNode).get()->AddEdgeToChildNode(std::move(edge)); //pass raw pointer
                        }
